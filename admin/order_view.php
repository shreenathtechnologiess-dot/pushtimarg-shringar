...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'save_order') {
    ...
    for ($i = 0; $i < count($item_ids); $i++) {
      ...
      $stmt = $conn->prepare("UPDATE order_items SET quantity = ?, unit_price = ? WHERE id = ? AND order_id = ?");
      if ($stmt) {
    ...
    if (count($item_ids) === 0) {
      $res = $conn->query("SELECT SUM(unit_price * quantity) AS s FROM order_items WHERE order_id = " . (int)$id);
...
// items: ensure unit_price and quantity columns exist in your table
$stmt = $conn->prepare("SELECT id, order_id, name, unit_price, quantity FROM order_items WHERE order_id = ?");
$stmt->bind_param('i', $id);
$stmt->execute();
$res = $stmt->get_result();
